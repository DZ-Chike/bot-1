...
const dataBot = {
    ...
    onclose() {
        if (this.isConnected) {
            this.isConnected = false;
            this.connect();
            logger.error('[SERVER] DataBot ON!')
        }
    },
    handleBuffer(buffer) {
        ...
        switch (reader.readUint8()) {
            ...
            case 241:
                ...
                logger.good('[SERVER] DataBot OFF!');
                break
        }
    }
};
...
class Bot {
    ...
    getClosestEntity(type, botX, botY, botSize) {
        ...
        for (const entity of Object.values(this.viewportEntities)) {
            ...
            switch (type) {
                case 'biggerPlayer':
                    isConditionMet = !entity.isVirus && !entity.isPellet && entity.size > botSize * 0.5 && entity.name !== this.name;
                    break;
                case 'pellet':
