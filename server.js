...
if (config.server.update) {
    requester(config.server.link, (err, req, data) => {
        ...
        if (config.server.version < requesterConfig.server.version) {
            logger.warn(`[SERVER]  new update was found.!`)
            logger.warn(`[SERVER] Descargar -> https://github.com/DZ-Chike/1_test`)
			process.exit();
        } else {
    ...
    logger.error('[SERVER] The update is false!')
}
...
class Bot {
            ...
            case 32:
                ...
                if (!this.isAlive) {
                    ...
                    if (!user.startedBots) {
                        ...
                        logger.good('[SERVER]  Bots Moving!')
                    }
    ...
    getClosestEntity(type, botX, botY, botSize) {
        ...
        for (const entity of Object.values(this.viewportEntities)) {
            ...
            switch (type) {
                case 'biggerPlayer':
                    isConditionMet = !entity.isVirus && !entity.isPellet && entity.size > botSize * 0.5 && entity.name !== bots.name
                    break
                case 'pellet':
